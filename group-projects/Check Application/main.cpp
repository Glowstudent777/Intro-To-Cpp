...
int main()
{
	...
	int payCommas = getCommas(pay);
	// int payCommas = (to_string(pay).length() - 7) / 3;
	...
	int topNbottom = (spacing - letters + payCommas + topNbottom + beforeName.length() + endSpacing + ((payEndExtra > employeeEndExtra) ? (employee.length() + 9 + employeeEndExtra) : (payEndExtra < employeeEndExtra) ? (to_string(pay).length() + 1 + payEndExtra) : (payEndExtra == employeeEndExtra)	? (employee.length() + 9 + employeeEndExtra) : endSpacing));
